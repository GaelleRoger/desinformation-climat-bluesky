...
| BigQuery direct en bronze | Requêtable immédiatement | Stockage plus cher pour du brut peu interrogé, couplé au moteur | Valable mais démo GCS voulue pour le portfolio |

**Décision :** bronze = fichiers JSON dans GCS, pour les posts comme pour la météo (`bronze-posts`, `bronze-weather`). Le job d'ingestion écrit un fichier horodaté par exécution (`dt=YYYY-MM-DD/posts_HHMM.json`). BigQuery lit le bronze via **tables externes** (pas de duplication de données). Un troisième bucket, `vertex-staging`, utilise la même techno (stockage objet) mais **n'appartient pas à la couche bronze** : c'est une zone de transit éphémère pour les échanges avec Vertex. 
...
**Décision :** Dataform fait le travail de dbt (modèles SQL versionnés, dépendances, tests) tout en étant intégré et gratuit dans GCP. 
...
Un appel quotidien à la météo **courante** de Paris suffit et reste dans le plan gratuit d'OpenWeatherMap (via API)
...
| Cloud Composer (Airflow managé) | Standard du marché, écosystème riche | Cluster permanent → plusieurs centaines d'€/mois même au repos | ❌ rédhibitoire avec 250 € d'essai |
...
1. **Budget alert** GCP à 50 €, 100 €, 200 € (notification email)
2. **Fréquence des jobs** : 15 min, ajuster selon le volume réel observé.
